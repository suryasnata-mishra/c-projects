...
void deposit_money() {
  ...
  while (fread(&acc_r, sizeof(acc_r), 1, file)) {  // Timestamp 4:01:00
  ...
  fclose(file);
  printf("\nMoney could not be deposited as the Account no %d was not found in records.", acc_no);
}
...
void check_balance() {
  ...
  while(fread(&acc_read, sizeof(acc_read), 1, file)) {
    if (acc_read.acc_no == acc_no) {
      printf("\nWelcome %s ! Your current balance is Rs. %.2f", acc_read.name , acc_read.balance );
      fclose(file);
      return;
    }
  }
